...
template <typename TIPO>class lista{
	...
	private:
		struct nodo{
			...
			nodo* siguiente;
			nodo* anterior;
			nodo(TIPO elem,nodo *pos=nullptr,nodo *pos2=nullptr): elemento(elem), siguiente(pos),anterior(pos){};
		};
...
void lista<TIPO>::insertar(const TIPO&x, lista<TIPO>::posicion pos){
	pos->siguiente=new nodo(x,pos->siguiente,pos);
	pos->siguiente->siguiente->anterior=pos->siguiente;
	if(pos==L){
		L=pos->siguiente;
	}
}
